...
@Component({
  selector: 'app-fichasaprobadas',
  standalone: true,
  imports: [CommonModule, FormsModule, SlicePipe, RouterModule ],
  templateUrl: './fichasaprobadas.component.html',
  styleUrl: './fichasaprobadas.component.css'
})
export class FichasaprobadasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.usuarioSesion && this.usuarioSesion.rut_enviado) {
      ...
      this.apiService.obtenerFichasPorusuarioyestado(rutUsuario, "APROBADA")
        .subscribe({
          next: (data) => {
